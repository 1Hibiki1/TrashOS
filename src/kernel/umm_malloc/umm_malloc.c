...
// const uint32_t x = (uint32_t)&_ebss;
// void* UMM_MALLOC_CFG_HEAP_ADDR = (void*)((x & ~(0b11)));
...
static void umm_free_core(void *ptr) {
    ...
    // uint32_t blockSize = (UMM_NBLOCK(c) & UMM_BLOCKNO_MASK) - c;
    // uint32_t blockSize = (UMM_NBLOCK(c) - c);
...
void umm_free(void *ptr) {
    // uint16_t c = (((uint8_t *)ptr) - (uint8_t *)(&(UMM_HEAP[0]))) / UMM_BLOCKSIZE;
